<footer class="border-t-2 border-vblue-dark bg-vblue text-white">
	<div class="p-5">
		<p class="font-bold mt-4">Project</p>
		<div class="grid md:grid-cols-4 grid-cols-1 md:justify-items-center items-center md:gap-0 gap-2">
			<p class="md:col-span-2">
				Verification DAO work in collaboration with <a
					href="https://kleros.io/"
					rel="external noopener"
					target="_blank">Kleros</a
				>
				court to gather and display NFT collections that have been verified through the Kleros court
				verification process.
				<a href="/about" sveltekit:prefetch>Read more</a>
			</p>
			<div class="flex flex-col">
				<a href="/about" sveltekit:prefetch>Get involved</a>
				<a href="/supported-registries" sveltekit:prefetch>Supported registries</a>
				<a href="/collections/submit" sveltekit:prefetch>Submit a collection</a>
			</div>
			<a href="/about#join" sveltekit:prefetch>Contact Verification DAO</a>
		</div>
	</div>
</footer>
